...
class PolicyBuilder:
    ...
    def build_att_policy(agent: Any) -> Tuple[nn.Module, torch.optim.Optimizer]:
        ...
        return policy, optimizer
    @staticmethod
    def build_flag_real_policy(agent):

        print("\n=== build_flag_real_policy (FLAG PAPER) ===")

        policy = FlagActorCriticReal(
...
if __name__ == "__main__":
    ...
    config.simulation.p_net_setting_num_nodes = 5

    config.nn = SimpleNamespace()
    config.nn.embedding_dim = 128
    config.nn.dropout_prob = 0.0
    config.nn.batch_norm = False
    ...
    config.rl.feature_constructor = SimpleNamespace()

    config.rl.feature_constructor.num_extracted_p_node_attrs = 2
    config.rl.feature_constructor.num_extracted_p_link_attrs = 1
    config.rl.feature_constructor.num_extracted_v_node_attrs = 2
    config.rl.feature_constructor.num_extracted_v_link_attrs = 1

    config.rl.feature_constructor.if_use_node_status_flags = False
    config.rl.feature_constructor.if_use_aggregated_link_attrs = False
