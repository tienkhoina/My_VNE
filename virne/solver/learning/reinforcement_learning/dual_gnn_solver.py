...
from virne.solver.learning.reinforcement_learning.flag_env import FlagInstanceEnv
...
def obs_as_tensor_for_flag(obs, device):
    ...
    if isinstance(obs, list):
        ...
        for o in obs:
            ...
            p_list.append(o["p_net"])
            v_list.append(o["v_net"])

            curr = o["curr_v_node_id"]

            if not torch.is_tensor(curr):
                curr = torch.tensor([curr], dtype=torch.long)
    ...
    p_batch = Batch.from_data_list([obs["p_net"]]).to(device)
    v_batch = Batch.from_data_list([obs["v_net"]]).to(device)

    curr = obs["curr_v_node_id"]
    ...
    mask = torch.tensor(obs["action_mask"]).to(device)
...
class FlagInstanceEnv(JointPRStepInstanceRLEnv):
    """
    RL env for FLAG
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with open_dict(config):
            ...
            config.rl.feature_constructor.name = "flag"
